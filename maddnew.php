...
{
  if (!$con->query("insert into useraccounts (name,IFSC,accountNo,accountType,city,address,email,password,balance,source,number,branch,age,monthly_income) values ('$_POST[name]','$_POST[IFSC]','$_POST[accountNo]','$_POST[accountType]','$_POST[city]','$_POST[address]','$_POST[email]','$_POST[password]','$_POST[balance]','$_POST[source]','$_POST[number]','$_POST[branch]','$_POST[age]','$_POST[monthly_income]')")) {
    ...
    <table class="table">
      <tbody>
        <tr>
          <form method="POST">
          <th>Name</th>
          <td><input type="text" name="name" class="form-control input-sm" pattern="[a-zA-Z' ']{5,20}" title="Enter alphabets only" required></td>
          <th>IFSC</th>
          <td><input type="text" name="IFSC" class="form-control input-sm" required></td>
        </tr>
        <tr>
        ...
        <tr>
          <th>Email</th>
          <td><input type="email" name="email" class="form-control input-sm" pattern="^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$" title="Enter an email-id" required></td>
          <th>Password</th>
        ...
        <tr>
          ...
          <td><input type="text" name="number"  class="form-control input-sm" pattern="[0-9]{10,10}" title="Enter ten digit only" required></td>
          <th>Branch</th>
          <td>
        ...
        <tr>
          <th>Age</th>
          <td><input type="number" name="age" class="form-control input-sm" required></td>
          <th>Monthly income</th>
          <td><input type="number" name="monthly_income" class="form-control input-sm" required></td>
        </tr>
        <tr>
          <td colspan="4">
